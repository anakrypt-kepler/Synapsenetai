...
std::string handleRpcNodeTorControl(const std::string& paramsJson) {
    ...
    bool initNetwork() {
        if (!config_.testnet && !config_.regtest && !config_.dev) {
            auto pqcStatus = quantum::getPQCBackendStatus();
            if (!pqcStatus.kyberReal || !pqcStatus.dilithiumReal || !pqcStatus.sphincsReal) {
                utils::Logger::error(
                    "PQC backend is simulated (liboqs unavailable). "
                    "Mainnet connection is prohibited with simulated PQC. "
                    "Use --testnet or build with liboqs support.");
                return false;
            }
        }
