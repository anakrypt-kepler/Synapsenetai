...
bool SessionCrypto::rotateKey(const std::string& sessionId) {
    ...
    std::vector<uint8_t> freshRandom(32);
    impl_->fillRandom(freshRandom.data(), freshRandom.size());

    std::vector<uint8_t> ikm;
    ikm.reserve(it->second.size() + freshRandom.size());
    ikm.insert(ikm.end(), it->second.begin(), it->second.end());
    ikm.insert(ikm.end(), freshRandom.begin(), freshRandom.end());

    auto newKey = crypto::sha256(ikm.data(), ikm.size());

    OPENSSL_cleanse(it->second.data(), it->second.size());
    OPENSSL_cleanse(ikm.data(), ikm.size());
    OPENSSL_cleanse(freshRandom.data(), freshRandom.size());

    it->second = std::vector<uint8_t>(newKey.begin(), newKey.end());
