...
struct RpcServer::Impl {
    ...
    uint64_t lastRateLimitCleanup = 0;
    static constexpr uint64_t RATE_LIMIT_CLEANUP_INTERVAL = 300;
    static constexpr size_t MAX_RATE_LIMIT_ENTRIES = 100000;
};
...
std::string RpcServer::Impl::formatResponse(const RpcResponse& response) {
    ...
    if (response.errorCode != 0) {
        ...
        ss << ",\"message\":\"" << escapeJsonString(response.errorMessage) << "\"}";
    } else {
...
void RpcServer::Impl::cleanupRateLimits() {
    ...
    for (auto it = rateLimits.begin(); it != rateLimits.end();) {
        if (now - it->second.windowStart >= static_cast<uint64_t>(rateLimitWindow) * 2) {
    ...
    for (auto it = authTokenRateLimits.begin(); it != authTokenRateLimits.end();) {
        if (now - it->second.windowStart >= static_cast<uint64_t>(rateLimitWindow) * 2) {
            it = authTokenRateLimits.erase(it);
        } else {
            ++it;
        }
    }
