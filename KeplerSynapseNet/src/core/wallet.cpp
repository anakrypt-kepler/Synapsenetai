...
static bool saveEncryptedSeedWords(const std::string& path,
    ...
    auto derived = pbkdf2_hmac(password, salt, 100000, 32);
    ...
    if (1 != EVP_EncryptInit_ex(ctx, nullptr, nullptr, aesKey.data(), iv.data())) {
        EVP_CIPHER_CTX_free(ctx);
        return false;
    }
...
bool Wallet::load(const std::string& path, const std::string& password) {
    ...
    const uint8_t MAGIC = 0xA5;
    if (encrypted.size() >= 2 && encrypted[0] == MAGIC && encrypted[1] == 2) {
        if (encrypted.size() < 2 + 16 + 12 + 16) return false;
        size_t pos = 2;
        std::vector<uint8_t> salt(encrypted.begin() + pos, encrypted.begin() + pos + 16); pos += 16;
        std::vector<uint8_t> iv(encrypted.begin() + pos, encrypted.begin() + pos + 12); pos += 12;
        std::vector<uint8_t> tag(encrypted.begin() + pos, encrypted.begin() + pos + 16); pos += 16;
        std::vector<uint8_t> ciphertext(encrypted.begin() + pos, encrypted.end());
        ...
        if (1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, 12, nullptr)) { EVP_CIPHER_CTX_free(ctx); return false; }
        if (1 != EVP_DecryptInit_ex(ctx, nullptr, nullptr, aesKey.data(), iv.data())) { EVP_CIPHER_CTX_free(ctx); return false; }
        ...
        if (1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, 16, tag.data())) { EVP_CIPHER_CTX_free(ctx); return false; }
