...
static std::string readString(const uint8_t* p, size_t& off, size_t maxLen, size_t dataSize) {
    if (off + 4 > dataSize) return std::string();
    uint32_t len = readU32(p + off);
    off += 4;
    if (len > maxLen || off + len > dataSize) return std::string();
...
static Validator deserializeValidator(const std::vector<uint8_t>& data) {
    Validator v;
    size_t minSize = crypto::PUBLIC_KEY_SIZE + 4 + 8 + 8 + 4 + 8 + 1 + 1 + 8;
    if (data.size() < minSize) return v;
    size_t off = 0;
    size_t sz = data.size();
    if (off + crypto::PUBLIC_KEY_SIZE > sz) return v;
    std::memcpy(v.pubKey.data(), data.data() + off, crypto::PUBLIC_KEY_SIZE); off += crypto::PUBLIC_KEY_SIZE;
    v.address = readString(data.data(), off, 1024, sz);
    if (off + 8 > sz) return v;
...
static ValidationResult deserializeResult(const std::vector<uint8_t>& data) {
    ValidationResult r;
    size_t required = 8 + 1 + 4*4 + 8 + 8 + 8 + 8;
    if (data.size() < required) return r;
    size_t off = 0;
    size_t sz = data.size();
    if (off + 8 > sz) return r;
    r.eventId = readU64(data.data() + off); off += 8;
    if (off + 1 > sz) return r;
    r.state = static_cast<ConsensusState>(data[off++]);
    if (off + 16 > sz) return r;
    ...
    if (off + 8 > sz) return r;
    uint64_t avgBits = readU64(data.data() + off); off += 8;
    std::memcpy(&r.averageScore, &avgBits, sizeof(double));
    if (off + 8 > sz) return r;
...
uint64_t Consensus::getTotalStake() const {
    ...
    uint64_t total = 0;
    for (const auto& [addr, val] : impl_->validators) {
        total = addSaturatingU64(total, val.stake);
    }
    return total;
}
...
bool Consensus::rewardValidator(const std::string& address, uint64_t amount) {
    ...
    it->second.stake = addSaturatingU64(it->second.stake, amount);
    it->second.totalRewards = addSaturatingU64(it->second.totalRewards, amount);
    saveValidatorToDb(impl_.get(), address, it->second);
    return true;
}
...
std::string Consensus::selectProposer() const {
    ...
    for (const auto& [addr, val] : impl_->validators) {
        totalStake = addSaturatingU64(totalStake, val.stake);
    }
    ...
    uint64_t cumulative = 0;
    for (const auto& [addr, val] : impl_->validators) {
        cumulative = addSaturatingU64(cumulative, val.stake);
...
bool Consensus::proposeBlock(const Block& block, const std::string& proposer) {
...

    {
        ...
        callback = impl_->onProposalCallback;
    }

    submitForValidation(eventId, pubKey);

    if (callback) {
        callback(std::to_string(eventId));
    }
    return true;
}
...
bool Consensus::voteOnProposal(const std::string& proposalId,
    ...
    {
        std::lock_guard<std::mutex> lock(impl_->mtx);
        auto valIt = impl_->validators.find(validator);
        if (valIt == impl_->validators.end()) return false;

        v.eventId = eventId;
        v.validator = valIt->second.pubKey;
        v.type = approve ? VoteType::APPROVE : VoteType::REJECT;
        v.scoreGiven = 1.0;
        v.timestamp = std::time(nullptr);
        callback = impl_->onVoteCallback;
    }

    if (!v.verify()) return false;

    bool accepted = vote(v);
    if (!accepted) return false;

    if (callback) callback(proposalId, approve);
    return true;
}
