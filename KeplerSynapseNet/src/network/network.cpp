...
static bool resolveHostWithTimeout(const std::string& host, uint16_t port,
                                    ...
                                    int timeoutMs = 3000) {
    auto addr = std::make_shared<sockaddr_storage>();
    auto len = std::make_shared<socklen_t>(0);
    auto fut = std::async(std::launch::async, [host, port, addr, len]() {
        return resolveHost(host, port, addr.get(), len.get());
    });
    ...
    bool result = fut.get();
    if (result) {
        std::memcpy(outAddr, addr.get(), sizeof(sockaddr_storage));
        *outLen = *len;
    }
    return result;
}
...
struct Network::Impl {
    ...
    mutable std::shared_mutex peersMtx;
    mutable std::mutex banMtx;
    mutable std::mutex rxMtx;
    mutable std::mutex outboundMtx;
    ...
    std::atomic<uint64_t> evictedPeers{0};
    std::atomic<uint64_t> tempBans{0};
    std::atomic<uint64_t> malformedMessages{0};
    std::atomic<uint64_t> rateLimitedEvents{0};
    std::atomic<uint64_t> overloadTransitions{0};
    std::atomic<bool> overloadMode{false};
    ...
    std::atomic<uint64_t> zombieDisconnects{0};
    ...
    void pruneExpiredBans(uint64_t now) {
        std::lock_guard<std::mutex> lock(banMtx);
    ...
    void updateOverloadMode(uint64_t now) {
        (void)now;
        if (!config.enableOverloadMode) {
            if (overloadMode.load()) {
                overloadMode = false;
        ...
        size_t peerSize;
        {
            std::shared_lock<std::shared_mutex> lock(peersMtx);
            peerSize = peers.size();
        }
        const uint64_t peerPercent = config.maxPeers == 0
            ? 0
            : static_cast<uint64_t>(peerSize * 100ULL / config.maxPeers);
        const uint64_t buffered = bufferedRxBytes();
    ...
    std::vector<scale::PeerSnapshot> buildPeerSnapshots() const {
        std::shared_lock<std::shared_mutex> lock(peersMtx);
    ...
    std::optional<Peer> removePeer(const std::string& peerId) {
        std::unique_lock<std::shared_mutex> pLock(peersMtx);
        ...
        pLock.unlock();
        {
            std::lock_guard<std::mutex> rLock(rxMtx);
            rx.erase(peerId);
        }
        {
            std::lock_guard<std::mutex> oLock(outboundMtx);
            outboundQueues.erase(peerId);
            outboundQueueBytes.erase(peerId);
        }
        return removed;
    }

    uint32_t addPenaltyAndMaybeBan(const std::string& address, double deltaPenalty, uint64_t now) {
...
void Network::Impl::acceptLoop() {
    while (running) {
        ...
        const uint64_t now = static_cast<uint64_t>(std::time(nullptr));
        pruneExpiredBans(now);
        updateOverloadMode(now);

        {
            std::lock_guard<std::mutex> bLock(banMtx);
        ...
        if (config.adaptiveAdmission) {
            ...
            limits.overloadMode = overloadMode.load();
            limits.enableDeterministicEviction = config.deterministicEviction;

            auto decision = scale::evaluateInboundAdmission(buildPeerSnapshots(), addr, limits);
            allow = decision.accept;
            evictPeerId = decision.evictPeerId;
        } else {
            std::shared_lock<std::shared_mutex> pLock(peersMtx);
        ...
        if (!evictPeerId.empty()) {
            auto removed = removePeer(evictPeerId);
            if (removed) {
                evicted = removed;
                evictedPeers += 1;
            } else {
    ...
    while (running) {
        ...
        for (size_t i = 0; i < fds.size(); i++) {
            if (fds[i].revents & (POLLHUP | POLLERR | POLLNVAL)) {
                auto dropped = removePeer(peerIds[i]);
            ...
            int peerSocket = -1;
            {
                ...
                peerSocket = pit->second.socket;
                peerAddress = pit->second.address;
            }

            const uint64_t nowSec = static_cast<uint64_t>(std::time(nullptr));
            const uint64_t nowMs = monotonicMs();
            pruneExpiredBans(nowSec);
            updateOverloadMode(nowSec);
            ...
            if (fds[i].revents & POLLOUT) {
                ...
                if (oqit != outboundQueues.end()) {
                    ...
                    while (!dq.empty()) {
                        ...
                        if (sendRaw(peerSocket, buf)) {
                            {
                                std::unique_lock<std::shared_mutex> pLock(peersMtx);
                                auto pit = peers.find(peerIds[i]);
                                if (pit != peers.end()) pit->second.bytesSent += buf.size();
                            }
...

            {
                ...
                for (;;) {
                    ssize_t n = ::recv(peerSocket, tmp.data(), tmp.size(), 0);
                    if (n > 0) {
...

                        {
                            ...
                            auto pit = peers.find(peerIds[i]);
                            if (pit != peers.end()) {
                                pit->second.lastSeen = nowSec;
                                pit->second.bytesRecv += static_cast<uint64_t>(n);
                            }
                        }
            ...
            if (dropAction != DropAction::NONE) {
                droppedPeer = removePeer(peerIds[i]);
                if (droppedPeer && dropAction == DropAction::TEMP_BAN) {
                    addPenaltyAndMaybeBan(
                        droppedPeer->address,
...
void Network::stop() {
    ...
    {
        std::unique_lock<std::shared_mutex> pLock(impl_->peersMtx);
    ...
    {
        std::lock_guard<std::mutex> rLock(impl_->rxMtx);
        impl_->rx.clear();
    }
...
bool Network::connect(const std::string& address, uint16_t port) {
    ...
    {
        const uint64_t now = static_cast<uint64_t>(std::time(nullptr));
        impl_->pruneExpiredBans(now);
        impl_->updateOverloadMode(now);
        {
            std::lock_guard<std::mutex> bLock(impl_->banMtx);
        ...
        {
            std::shared_lock<std::shared_mutex> pLock(impl_->peersMtx);
        ...
        if (impl_->config.adaptiveAdmission) {
            ...
            limits.overloadMode = impl_->overloadMode.load();
            limits.enableDeterministicEviction = false;
            std::string reason;
            if (!scale::allowOutboundConnection(impl_->buildPeerSnapshots(), address, limits, &reason)) {
...

    {
        ...
        if (impl_->config.adaptiveAdmission) {
            ...
            for (const auto& kv : impl_->peers) {
                if (kv.second.isOutbound) outCount++;
            }
            if (outCount >= impl_->config.maxOutbound) {
                close(sock);
                return false;
            }
        }
...
void Network::disconnect(const std::string& peerId) {
    auto removed = impl_->removePeer(peerId);
    impl_->updateOverloadMode(static_cast<uint64_t>(std::time(nullptr)));
...
void Network::ban(const std::string& peerId, uint32_t seconds) {
    ...
    if (!addr.empty()) {
        ...
        {
            std::lock_guard<std::mutex> bLock(impl_->banMtx);
            impl_->bannedPeers[addr] = std::time(nullptr) + seconds;
        }
        impl_->updateOverloadMode(static_cast<uint64_t>(std::time(nullptr)));
        return;
    }
...
bool Network::send(const std::string& peerId, const Message& msg) {
    ...
    int peerSocket = -1;
    {
        ...
        peerSocket = it->second.socket;
    }
    if (impl_->sendRaw(peerSocket, data)) {
        std::unique_lock<std::shared_mutex> pLock(impl_->peersMtx);
        auto it = impl_->peers.find(peerId);
        if (it != impl_->peers.end()) it->second.bytesSent += data.size();
        return true;
    }

    {
        std::lock_guard<std::mutex> oLock(impl_->outboundMtx);
        ...
        if (qbytes > impl_->config.maxOutboundQueueBytes) {
            ...
            impl_->outboundQueues.erase(peerId);
            impl_->outboundQueueBytes.erase(peerId);
        } else {
            return true;
        }
    }
...
NetworkStats Network::getStats() const {
    ...
    stats.bytesSent = impl_->bytesSent.load();
    stats.bytesReceived = impl_->bytesReceived.load();
    stats.messagesSent = impl_->messagesSent.load();
    stats.messagesReceived = impl_->messagesReceived.load();
    stats.uptime = std::time(nullptr) - impl_->startTime;
    stats.overloadMode = impl_->overloadMode.load();
    stats.bufferedRxBytes = impl_->bufferedRxBytes();
    stats.rejectedConnections = impl_->rejectedConnections.load();
    stats.evictedPeers = impl_->evictedPeers.load();
    stats.tempBans = impl_->tempBans.load();
    stats.malformedMessages = impl_->malformedMessages.load();
    stats.rateLimitedEvents = impl_->rateLimitedEvents.load();
    stats.overloadTransitions = impl_->overloadTransitions.load();
    stats.zombieDisconnects = impl_->zombieDisconnects.load();
    return stats;
}
