...
bool Keys::fromMnemonic(const std::string& mnemonic, const std::string& passphrase) {
    ...
    if (1 != PKCS5_PBKDF2_HMAC(mnemonic.c_str(), static_cast<int>(mnemonic.size()),
                               ...
                               2048, EVP_sha512(), 64, impl_->seed.data())) {
        return false;
    }
...
bool Keys::validateMnemonic(const std::string& mnemonic) {
    ...
    if (mnemonicWords.size() != 12 && mnemonicWords.size() != 15 &&
        mnemonicWords.size() != 18 && mnemonicWords.size() != 21 && mnemonicWords.size() != 24) {
        return false;
    }

    return validateMnemonicChecksum(mnemonic);
}

bool Keys::validateMnemonicChecksum(const std::string& mnemonic) {
    std::istringstream iss(mnemonic);
    std::string word;
    std::vector<std::string> words;
    const auto& list = bip39Wordlist();
    ...
    for (const auto& w : words) {
        ...
        if (it == wordIndex.end()) return false;
        int idx = it->second;
        for (int j = 10; j >= 0; j--) {
    ...
    uint32_t totalBits = static_cast<uint32_t>(words.size()) * 11;
    uint32_t checksumBits = totalBits / 33;
    uint32_t entropyBits = totalBits - checksumBits;
    uint32_t entropyBytes = entropyBits / 8;

    std::vector<uint8_t> entropy(entropyBytes, 0);
    for (uint32_t i = 0; i < entropyBits; i++) {
        if (bits[i]) {
            entropy[i / 8] |= (1 << (7 - (i % 8)));
        }
    }
    ...
    for (uint32_t i = 0; i < checksumBits; i++) {
        ...
        bool actual = bits[entropyBits + i];
