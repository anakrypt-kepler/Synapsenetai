<script lang="ts">
  ...
  async function saveConfig() {
    try {
      ...
      console.error("Failed to save NAAN config:", e);
    }
  }
</script>
...
<div class="content-area">
  ...
  <div class="card">
    <div class="form-group">
      <label for="naan-topics">Topic Preferences</label>
      <input id="naan-topics" type="text" bind:value={topicPreferences} placeholder="e.g. AI, cryptography, distributed systems" />
    </div>
    <div class="form-group">
      <label id="naan-sources-label">Research Sources</label>
      <div class="source-toggle" role="group" aria-labelledby="naan-sources-label">
        <button class="filter-btn" class:active={researchSources === "clearnet"} on:click={() => (researchSources = "clearnet")} aria-pressed={researchSources === "clearnet"}>Clearnet</button>
        <button class="filter-btn" class:active={researchSources === "tor"} on:click={() => (researchSources = "tor")} aria-pressed={researchSources === "tor"}>Tor</button>
        <button class="filter-btn" class:active={researchSources === "both"} on:click={() => (researchSources = "both")} aria-pressed={researchSources === "both"}>Both</button>
      </div>
    </div>
    <div class="form-group">
      <label for="naan-tick-interval">Tick Interval (seconds): {tickInterval}</label>
      <input id="naan-tick-interval" type="range" min="10" max="600" bind:value={tickInterval} />
    </div>
    <div class="form-group">
      <label for="naan-budget">Budget Limit (NGT per epoch)</label>
      <input id="naan-budget" type="text" bind:value={budgetLimit} />
    </div>
