<script lang="ts">
  ...
  import { validateBip39Phrase } from "../../lib/bip39";
  ...
  async function doRestore() {
    ...
    if (words.length !== 24) {
      restoreError = "Seed phrase must be exactly 24 words.";
      return;
    }
    const validation = validateBip39Phrase(restoreSeed);
    if (!validation.valid) {
      restoreError = `Invalid BIP39 words: ${validation.invalidWords.join(", ")}`;
      return;
    }
    try {
      ...
      restoreError = e.message || "Wallet restore failed.";
    }
  }
  ...
  async function startDownload() {
    ...
    downloadProgress = 0;
    try {
      const result = await rpcCall("model.download", JSON.stringify({ model: "llama-3b" }));
  ...
  async function selectLocalFile() {
    try {
      ...
      console.error("Failed to select model file:", e);
    }
  }
    ...
    <div class="wizard-body">
      {#if step === 1}
        <div class="step-content">
          ...
          {:else if walletMode === "create"}
            {#if !seedConfirmed}
              ...
              <div class="form-group">
                <label for="setup-wallet-password">Password (optional)</label>
                <input id="setup-wallet-password" type="password" bind:value={walletPassword} placeholder="Wallet password" />
              </div>
          ...
          {:else if walletMode === "restore"}
            ...
            <textarea class="seed-input" bind:value={restoreSeed} rows="4" placeholder="word1 word2 word3 ..." aria-label="Seed phrase input"></textarea>
            {#if restoreError}
              <div class="error-box">{restoreError}</div>
            {/if}
      ...
      {:else if step === 2}
        <div class="step-content">
          ...
          {#if connectionType === "tor_bridges"}
            <div class="form-group">
              <label for="setup-bridge-lines">Bridge Lines</label>
              <textarea id="setup-bridge-lines" bind:value={bridgeLines} rows="4" placeholder="obfs4 bridge lines, one per line"></textarea>
            </div>
          {/if}
        </div>
      ...
      {:else if step === 4}
        <div class="step-content">
          ...
          <div class="form-group">
            <label for="setup-cpu-threads">CPU Threads ({cpuThreads} / {systemInfo.cpu_cores})</label>
            <input id="setup-cpu-threads" type="range" min="1" max={systemInfo.cpu_cores} bind:value={cpuThreads} />
          </div>
          <div class="form-group">
            <label for="setup-ram-limit">RAM Limit ({ramLimitMb} MB / {systemInfo.ram_total_mb} MB)</label>
            <input id="setup-ram-limit" type="range" min="512" max={systemInfo.ram_total_mb} step="256" bind:value={ramLimitMb} />
          </div>
          <div class="form-group">
            <label for="setup-disk-limit">Disk Space Limit (MB)</label>
            <input id="setup-disk-limit" type="number" bind:value={diskLimitMb} min="1000" />
          </div>
          <div class="gpu-section">
            ...
            {#if gpuEnabled}
              {#if systemInfo.gpu_devices.length > 0}
                <div class="form-group">
                  <label for="setup-gpu-device">GPU Device</label>
                  <select id="setup-gpu-device" bind:value={gpuDevice}>
              ...
              {/if}
              <div class="form-group">
                <label for="setup-gpu-layers">GPU Layers (offload to VRAM): {gpuLayers}</label>
                <input id="setup-gpu-layers" type="range" min="0" max="64" bind:value={gpuLayers} />
              </div>
