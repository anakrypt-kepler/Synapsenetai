<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      console.error("Failed to subscribe to ai.stream:", e);
    }
  ...
  async function sendMessage() {
    ...
    try {
      const result = await aiComplete(text, web4Enabled || undefined);
      if (reqId !== currentRequestId) return;
      if (result && !streamReceivedTokens) {
        try {
          ...
          messages = [
            ...messages,
            { role: "assistant", content: result, timestamp: Date.now(), requestId: reqId },
          ];
        }
      }
    } catch (e: any) {
...
<div class="chat-panel">
  <div class="chat-header">
    ...
    <button
      ...
      aria-pressed={web4Enabled}
      aria-label="Toggle Web4 mode"
    >
      Web4
    </button>
  </div>
  <div class="chat-messages" bind:this={messagesContainer} role="log" aria-label="Chat messages" aria-live="polite">
  ...
  <div class="chat-input-area">
    ...
    ></textarea>
    <button class="send-btn btn-primary" on:click={sendMessage} disabled={streaming || !inputValue.trim()} aria-label="Send message">
      Send
    </button>
  </div>
</div>
