...
pub fn save_setup_config(config: SetupConfig) -> Result<(), String> {
    ...
    let mut wallet = toml::value::Table::new();
    wallet.insert("mode".into(), toml::Value::String(config.wallet_mode));
    if let Some(ref pwd) = config.password {
        if !pwd.is_empty() {
            wallet.insert("password_protected".into(), toml::Value::Boolean(true));
        }
    }
    table.insert("wallet".into(), toml::Value::Table(wallet));

    let mut connection = toml::value::Table::new();
    connection.insert("type".into(), toml::Value::String(config.connection_type));
    if let Some(ref bridges) = config.bridge_lines {
        if !bridges.is_empty() {
            connection.insert("bridge_lines".into(), toml::Value::String(bridges.clone()));
        }
    }
    table.insert("connection".into(), toml::Value::Table(connection));

    let mut ai = toml::value::Table::new();
    ai.insert("mode".into(), toml::Value::String(config.ai_model));
    if let Some(ref path) = config.model_path {
        if !path.is_empty() {
            ai.insert("model_path".into(), toml::Value::String(path.clone()));
        }
    }
    table.insert("ai".into(), toml::Value::Table(ai));

    let mut resources = toml::value::Table::new();
    resources.insert("cpu_threads".into(), toml::Value::Integer(config.cpu_threads as i64));
    resources.insert("ram_limit_mb".into(), toml::Value::Integer(config.ram_limit_mb as i64));
    resources.insert("disk_limit_mb".into(), toml::Value::Integer(config.disk_limit_mb as i64));
    resources.insert("gpu_enabled".into(), toml::Value::Boolean(config.gpu_enabled));
    if let Some(ref device) = config.gpu_device {
        if !device.is_empty() {
            resources.insert("gpu_device".into(), toml::Value::String(device.clone()));
        }
    }
    resources.insert("gpu_layers".into(), toml::Value::Integer(config.gpu_layers as i64));
    table.insert("resources".into(), toml::Value::Table(resources));

    let mut startup = toml::value::Table::new();
    startup.insert("launch_at_login".into(), toml::Value::Boolean(config.launch_at_startup));
    startup.insert("mine_background".into(), toml::Value::Boolean(config.mine_background));
    table.insert("startup".into(), toml::Value::Table(startup));
...
#[tauri::command]
pub fn ai_complete(prompt: String, web4: Option<bool>) -> Result<String, String> {
    let params = serde_json::json!({
        "prompt": prompt,
        "web4": web4.unwrap_or(false)
    })
    .to_string();
